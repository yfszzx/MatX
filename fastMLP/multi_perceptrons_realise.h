...
multi_perceptrons::multi_perceptrons(string name){
	g_gpu_init();
...
void multi_perceptrons::init_nervs(){			
	if(nervs!=NULL){
...
void multi_perceptrons::struct_init(){	
...
void multi_perceptrons::struct_settted(bool save){
	struct_init();
	weight_rand();
	struct_show();
	if(save)struct_save();
}
void multi_perceptrons::weight_rand(){
	for(int i=0;i<layers_num;i++)
		nervs[i]->weight_rand();
}
void multi_perceptrons::set_data_num(int dt_num){
...
float multi_perceptrons::get_result(float *input,float *target){
...
float * multi_perceptrons::layer_out(int layer,float *&s_input,int num,bool in_cuda_pos){//中间层的输出，返回中间层输出数据的指针(device)
	CUDA_CHECK;
	set_data_num(num);
	float *input;
	if(in_cuda_pos){
...
float multi_perceptrons::run(float *s_input,float *out,float *s_target,int num,float dropout,bool in_cuda_pos,bool out_cuda_pos){
	set_data_num(num);
	float *input;
	float *target;
	if(in_cuda_pos){
...
void multi_perceptrons::top_pre_deriv(){
	if(nerv_top->type=='t'){
...
void multi_perceptrons::cacul_nerv(float *input,float *target) {	
...
void multi_perceptrons::struct_show(){
...
bool multi_perceptrons::struct_read(){			
...
void multi_perceptrons::struct_save(int idx){
	coutd<<"正在保存"<<path;
...
void multi_perceptrons::reguler_set(){
	if(reguler_mod==0){
		coutd<<"是否使用正则化?(Y/N)";
		reguler_mod=0;		
		string sel;
...
void multi_perceptrons::loss_mod_set(){
	coutd<<"输入损失函数类型(1.L1 2.L2 c.互熵 s.soft_max):";
	cin>>loss_mod;
}
void multi_perceptrons::reguler_show(){
	coutd<<"各层正则模式（1:L1 2:L2 ) 和正则系数:";
	for(int i=0;i<layers_num;i++)
		coutd<<"\t"<<(i+1)<<" mod:"<<nervs[i]->decay_mod<<" decay:"<<nervs[i]->decay;

}
void multi_perceptrons::struct_set(int i_dimen,int o_dimen){
...
void multi_perceptrons::struct_simple_set(int in_dimen,int out_dimen,int nodes,char nodes_mod,char out_mod,char loss,float  decay,char decay_mod){
...
int multi_perceptrons::select_layer(string illu){
	int idx;
	do{		
		coutd<<"输入要修改的层序:";
		coutd<<illu;
...
void multi_perceptrons::struct_edit(){		
	struct_show();
	string sel;
	coutd<<"\t1.修改损失函数";
	coutd<<"\t2.修改正则模式";
...
multi_perceptrons::~multi_perceptrons(){
	if(nervs!=NULL){
		for(int i=0;i<layers_num;i++)
			delete nervs[i];
		delete [] nervs;
...
int multi_perceptrons::memery(bool show){
...
void multi_perceptrons::free_mem(){
	if(output_tmp!=NULL){
		cudaFree(output_tmp);
		cudaFree(tmp_array);
	};
